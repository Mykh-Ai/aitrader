...
### Phase 3 Replay Entry Modes and Explicit Binding Bridge

Phase 3 defines two explicit replay entry modes with non-overlapping roles:

1. **Research-Derived Replay Path (`RESEARCH_DERIVED`)**
   - Purpose: research screening, baseline replay, and hypothesis triage.
   - Source: deterministic ruleset formalization from research artifacts
     (`analyzer_setup_shortlist.csv`, `analyzer_research_summary.csv`) through
     the existing ruleset formalization flow.
   - Contract role: supports research iteration and does not claim that
     machine-generated mapping artifacts are executable truth.

2. **Explicit Binding Replay Path (`EXPLICIT_BINDING`)**
   - Purpose: strict replay from explicitly reviewed replay bindings.
   - Source: reviewed replay-ready binding artifact (see bridge model below).
   - Contract role: input boundary for strict validation replay, including
     `PHASE3_MAPPING_ONLY`.

#### Current formalization chain and intentional non-readiness

The implemented formalization chain currently materializes:
...
`phase3_ruleset_mapping.csv` as machine-generated by harvest/formalization remains an
intermediate artifact and may intentionally contain unresolved/manual-required fields
with statuses such as `PARTIAL` / `NOT_INTEGRATED`. In that state it is **not**
executable replay truth and must not be auto-promoted to replay-ready.

#### Missing transition layer: explicit binding bridge

The contract gap is not a validator/orchestrator bug; it is a missing architectural
transition layer between machine-generated partial mapping and strict replay-ready
binding.

This spec introduces a required explicit bridge layer (design-level in v1.0):

- **Bridge responsibility:** convert PARTIAL mapping intent into reviewed executable
  replay binding through explicit human/policy review.
- **Authority:** promotion from PARTIAL to READY is allowed only through explicit
  review/promotion authority (owner/reviewer workflow), never by Analyzer harvest,
  ruleset formalization defaults, or orchestrator auto-repair.
- **Audit model:** append-only, evidence-referenced decision trail.

#### Reviewed replay-ready artifact (recommended model)

Recommendation: introduce a separate reviewed artifact, e.g.
`phase3_ruleset_binding.csv`, instead of reusing machine-generated mapping as
executable truth. Rationale:

- preserves a hard boundary between generated hypothesis mapping and reviewed
  executable binding;
- prevents accidental semantic conflation of PARTIAL and READY states;
- improves auditability of promotion decisions and policy versions.

Minimum required READY fields in the reviewed binding artifact:
...
- explicit readiness/integration statuses (READY-class values).

Minimum review/audit metadata:
...
#### Strict replay gate ownership

`PHASE3_MAPPING_ONLY` (strict explicit binding mode) must consume only replay-ready
reviewed binding artifacts. It must not consume PARTIAL machine-generated mapping rows.
No auto-promotion, no validator weakening, and no orchestrator auto-fix behavior are
allowed in this contract.

#### Manifest semantics separation

Replay manifests must carry an explicit replay entry mode field to prevent semantic
mixing of results across paths:

- `ReplayEntryMode = RESEARCH_DERIVED`
- `ReplayEntryMode = EXPLICIT_BINDING`

This field is mandatory for future result aggregation and cross-run interpretation.
