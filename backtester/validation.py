...
def _expectancy_status(metrics_row: pd.Series) -> tuple[str, str]:
    ...
    if float(expectancy) > 0:
        return "PASS", f"expectancy={float(expectancy):.8f} (>0)"
    if float(expectancy) < 0:
        return "FAIL", f"expectancy={float(expectancy):.8f} (<0)"
    return "REVIEW", "expectancy=0"
...
def _drawdown_status(drawdown_df: pd.DataFrame | None) -> tuple[str, str]:
    ...
    if bases == ["RESOLVED_TRADE_COUNT"]:
        return "NOT_EVALUATED", "drawdown not evaluated: basis=RESOLVED_TRADE_COUNT is non-monetary"
    ...
    if float(max_drawdown) <= 0.10:
        return "PASS", f"provisional max_drawdown={float(max_drawdown):.6f} <= 0.10"
    if float(max_drawdown) <= 0.20:
        return "REVIEW", f"provisional max_drawdown={float(max_drawdown):.6f} in (0.10, 0.20]"
    return "FAIL", f"provisional max_drawdown={float(max_drawdown):.6f} > 0.20"
...
def _long_short_asymmetry_status(metrics_row: pd.Series) -> tuple[str, str]:
    ...
    if ratio > 4.0:
        return "FAIL", f"provisional count asymmetry ratio={ratio:.4f} (>4.0)"
    if ratio > 2.0:
        return "REVIEW", f"provisional count asymmetry ratio={ratio:.4f} (>2.0)"
    return "PASS", f"provisional count asymmetry ratio={ratio:.4f}"


def _source_concentration_status(scope_ledger_df: pd.DataFrame) -> tuple[str, str]:
    if "source_setup_id" not in scope_ledger_df.columns:
        return "NOT_EVALUATED", "source concentration not evaluated: source_setup_id unavailable"

    source_counts = (
        scope_ledger_df["source_setup_id"].astype(str).replace("", pd.NA).dropna().value_counts(sort=True)
    )
    if source_counts.empty:
        return "NOT_EVALUATED", "source concentration not evaluated: no source_setup_id values"

    max_share = float(source_counts.iloc[0] / source_counts.sum())
    if max_share > 0.80:
        return "FAIL", f"provisional source concentration max_share={max_share:.4f} (>0.80)"
    if max_share > 0.60:
        return "REVIEW", f"provisional source concentration max_share={max_share:.4f} (>0.60)"
    return "PASS", f"provisional source concentration max_share={max_share:.4f}"
...
def build_validation_artifacts(
    *,
    ...
    for scope in scopes:
        ...
        asymmetry_status, asymmetry_note = _long_short_asymmetry_status(metrics_row)
        source_status, source_note = _source_concentration_status(scope_ledger)
